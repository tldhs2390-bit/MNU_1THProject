 ...
 */
@WebServlet("/admin_guide_fruit_delete.do")
public class AdminGuideFruitDeleteServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AdminGuideFruitDeleteServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(idStr != null && !idStr.isEmpty()) {
            ...
            if(row > 0) {
                // 삭제 성공 시 목록 페이지로 이동
                response.sendRedirect(request.getContextPath() + "admin_guide_fruit_list.do");
            } else {
