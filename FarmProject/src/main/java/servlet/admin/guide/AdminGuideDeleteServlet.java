 ...
 */
@WebServlet("/admin_guide_delete.do")
public class AdminGuideDeleteServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AdminGuideDeleteServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(idStr != null && !idStr.isEmpty()) {
            ...
            if(row > 0) {
                // 삭제 성공 시 목록 페이지로 이동
                response.sendRedirect(request.getContextPath() + "admin_guide_list.do");
            } else {
